...
import { leagueTitle, parsePlayed } from "./format";
 ...
 *
 * The file is an export from `poe-char-export`, which reads the account's
 * characters tab — the same data the profile page shows — and writes one JSON
 * file. It is uploaded rather than fetched: the archive runs on a home server
 * behind the owner's own network and the endpoints are undocumented, anonymous
 * and rate-limited, so collection belongs on the machine that can reach them.
 *
 * What the export does not carry is the league a character belongs to. Every
 * character migrates to Standard when its league ends, so the league it is in
 * now says nothing about where it was played; the exporter guesses from the
 * last login time and grades its own guess, and the owner's record — which is
...
function stageExport(body: string): string {
...
function readStaged(token: string): PoeExport {
...
function planFor(userId: number, exported: PoeExport, token: string): ImportPlan {
  const rows: ImportRow[] = exported.characters.map((character) => {
    ...
    return {
      ...
      // Only offered where the exporter called it certain, which means the
...
export async function importPoeExportAction(_prev: ImportState, formData: FormData): Promise<ImportState> {
  ...
  const run = db.transaction(() => {
    for (const character of exported.characters) {
      if (!formData.get(`include:${character.name}`)) continue;
      ...
      const slug = text(formData, `league:${character.name}`);
      if (!slug) continue;
