...
src/lib/search.ts             whole-archive search: name, skill, unique in use
...
- **The export format is a contract.** `docs/export-format.md` documents every field
  ...
  `(game, slug)`, never by patch or id.
- **Search counts a unique only while it is in use**, by the same rule as the paper doll
  (`build.slots` or `build.treeJewels`), and never matches a rare by name.
